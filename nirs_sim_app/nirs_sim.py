...
def simulate(spec, wavelength):
    ...
    for seed in seeds:
        ...
        tofBins = np.digitize(cfg.prop[1:, 3] @ detp[2:(2+nmedia)] / c, tof_domain) - 1
        tofBins[tofBins == ntof] -= 1
        np.add.at(phiTD, (detBins, tofBins), np.exp(-cfg.prop[1:, 0] @ detp[2:(2+nmedia)]))
        photon_counts += np.bincount(detBins*ntof+tofBins, minlength=ndet*ntof).reshape((ndet, ntof))
        np.add.at(paths, (detBins, tofBins), detp[2:(2+nmedia)].T)
